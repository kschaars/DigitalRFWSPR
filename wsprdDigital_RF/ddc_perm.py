...
if __name__ == "__main__":
    ...
    #   channel name i.e. chD
    metadataPath = "/data/captures/" + sys.argv[1] + "/data"
    dir_drf = metadataPath
    metadataPath = metadataPath + "/" + sys.argv[5] + "/metadata"
    startTime = 0
    ...
    extract_and_decode_wspr(
        drf_dir= dir_drf,  
        channel=sys.argv[5] ,    #Name of digital RF directory, no "/"
        sched_df=sched,      
        use_gpu= sys.argv[3],    #Boolean for using the GPU
        decodeRange = ast.literal_eval(sys.argv[4]), #An array containing center frequency, lower, and upper decoding range
